...
%\begin{itemize}
%\item Let $K$ be the number of latent topics.
%\item Topic-Word distribution: $\phi_k$ is a discrete probability distribution over a fixed vocabulary that represents the $k$th topic distribution. \\ $\phi_k \sim \text{Dirichlet}(\beta)$
%
%\item $\alpha$ is the per-di
%\item Document-Topic distribution: $\theta_d$ is a document specific discrete probability distribution over a fixed number of topics. \\ $\theta_d \sim \text{Dirichlet}(\alpha)$
%\item $z_i$ is topic index for word $w_i$.
%\item From a document $\theta_d$ we select a topic $z_i$.
%\end{itemize}
%
%
%\begin{figure}[htp]
...
%
%  \end{tikzpicture}
%  \caption{Plate Diagram of LDA. For more information}
